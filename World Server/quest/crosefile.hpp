...
class CRoseFile
{
	...
	/** Contructor opening a file after contructing
        ...
        */
	CRoseFile( const CFILE_CHAR* fname, CFILE_CHAR mode )
	{
		fh       = NULL;
		ftype    = FT_NONE;
		fmode    = FM_NONE;
	...
	/** Opens a file
        ...
        */
	CFILE_BOOLEAN Open( const CFILE_CHAR* fname, CFILE_CHAR mode )
	{
		if ( fh != NULL )
			Close( );
		fmode                  = mode;
	...
	CFILE_CHAR* ReadLn( )
	{
		...
		if ( GetS( result, CFILE_MAXLINELENGHT ) == NULL )
		{
			...
			for ( CFILE_UINT i = 0; i < result->size( ); i++ )
				res[ i ] = 0x00;
