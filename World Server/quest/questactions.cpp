...
uint32_t GetRewardValue( uint32_t function, uint32_t amount, CPlayer* client, uint16_t nDupCNT )
{
	...
	case 1:
	{
		uint32_t tempVal = ( client->Stats->Level + 3 ) * amount;
...
QUESTREWD( 000 )
{
	...
	case 0:
	{
		...
		if ( client->ActiveQuest == data->iQuestSN )
			client->ActiveQuest = 0;
	...
	case 1:
	{
		...
		for ( uint32_t i = 0; i < 10; i++ )
		{
			...
			if ( client->quest.quests[ i ].QuestID != 0 )
				continue;
			memset( &client->quest.quests[ i ], 0, sizeof( SQuest ) );
...
QUESTREWD( 005 )
{
	...
	case 2: //Item
	{
		...
		nItem.lifespan   = 100;
		nItem.appraised  = 1;
		uint32_t slot    = client->AddItem( nItem );
		if ( slot == 0xffff ) // Fail
		{
...
QUESTREWD( 008 )
{
	...
	for ( uint32_t i = 0; i < data->iHowMany; i++ )
	{
...
QUESTREWD( 009 )
{
	...
	uint32_t hash     = MakeStrHash( tempName );
	if ( client->questdebug )
...
QUESTREWD( 012 )
{
    ...
    std::map<uint32_t, char*>::iterator triggerITR = server->NpcNames.find(thisNpc->SelectedNpc->NpcID);
    ...
    if(data->btMsgType == 1){
    	...
    	pakout.AddBytes((uint8_t*)npcName, strlen(npcName));
    	pakout.Add<uint8_t>('>');
    	pakout.Add<string>(sayStr);
    	server->SendPacketToZone(thisNpc, &pakout);
    }
