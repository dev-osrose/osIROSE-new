...
CDrop* CWorldServer::GetDrop( CMonster* thismon )
{
	try
	{
		...
		if ( newdrop->item.durability > 100 )
			newdrop->item.durability = 100;
		if ( newdrop->item.lifespan > 100 )
			newdrop->item.lifespan = 100;
		newdrop->item.gem          = 0;
		return newdrop;
	}
...
{ //if droptype = 1 then it is a normal drop. if it is 2 then it is a potential side drop.
	...
	if ( newdrop->item.itemtype == 10 || newdrop->item.itemtype == 12 )
	{
		...
		if ( thismon->thisnpc->level == 1 && newdrop->item.count > 6 )
			newdrop->item.count = 6; //limit the drop rate of items from level 1 event mobs
