...
*/ if ( Stats->Level > 203 )
	...
	unsigned lvlexp[] = {0, 184, 294, 436, 617, 840, 1108, 1428, 1801, 2234,
	                     ...
	                     1425600000, 1567889323, 1719696384, 1881429525, 2053507456, 0xffffffff};
...
{
	char text[ 50 ];
	sprintf_s( text, 50, "Attack: %i | Critical: %i", Stats->Attack_Power, Stats->Critical );
	...
	sprintf_s( text, 50, "Defense: %i | Magic Defense: %i", Stats->Defense, Stats->Magic_Defense );
	...
	sprintf_s( text, 50, "Accury: %i | Dodge: %i", Stats->Accury, Stats->Dodge );
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
	...
	sprintf_s( text, 50, "aspeed: %i | mspeed: %i", Stats->Attack_Speed, Stats->Move_Speed );
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
	...
	sprintf_s( text, 50, "Position[%i]: (%.0f,%.0f)", Position->Map, Position->current.x, Position->current.y );
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
	...
	sprintf_s( text, 50, "ClientID: %u | CharID: %u", clientid, CharInfo->charid );
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
	...
	sprintf_s( text, 50, "inGame: %i | Logged: %i", Session->inGame, Session->isLoggedIn );
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
	...
	sprintf_s( text, 50, "ClanName[%u]: %s | ClanGrade: %i | ClanRank: %i", Clan->clanid, Clan->clanname, Clan->grade, Clan->clanrank );
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
...
int CPlayer::ExecuteQuestTrigger( uint32_t hash )
{
	...
	for ( uint32_t i = 0; i < trigger->ConditionCount; i++ )
	{
		int command = trigger->Conditions[ i ]->opcode;
		if ( command > 30 || command < 0 )
			continue;
	...
	for ( uint32_t i = 0; i < trigger->ActionCount; i++ )
	{
...
SQuest* CPlayer::GetActiveQuest( )
{
	...
	{
		if ( quest.quests[ i ].QuestID != ActiveQuest )
			continue;
	...
	if ( newslot != 0xffff )
	{
		if ( items[ newslot ].count > 0 )
		{
			...
			if ( ntcount + utcount > 999 )
			{
				...
				if ( otherslot != 0xffff )
				{
					if ( items[ otherslot ].count != 0 )
						items[ otherslot ].count += item.count;
					else
						items[ otherslot ] = item;
				...
				items[ newslot ].count = ntcount + utcount;
		}
		else
			items[ newslot ] = item;
	}
