...
class CCharacter
{
	...
	int  GetCritRate( );
	int  GetAtkDmg( CCharacter* pDEF, uint16_t wHitCNT, int iSuc );
	int  GetMgcDmg( CCharacter* pDEF, uint16_t wHitCNT, int iSuc );
	...
	bool IsSummon( );
	bool IsOnBattle( );
	void StartAction( CCharacter* Target, uint8_t action, uint32_t skillid = 0 );
