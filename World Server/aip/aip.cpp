...
void CWorldServer::ReadAIP( std::string path, uint32_t index )
{
	...
	{ // goto done;
		...
		for ( uint32_t i = 0; i < BlockCount; i++ )
		{
			...
			for ( uint32_t j = 0; j < RecordCount; j++ )
			{
				...
				script->ConditionCount = fh->Get< uint32_t >( );
				if ( script->ConditionCount > 0 )
				{
					script->Conditions = new CAip::SAipDatum*[ script->ConditionCount ];
					for ( uint32_t k = 0; k < script->ConditionCount; k++ )
					{
						CAip::SAipDatum* data = new CAip::SAipDatum( );
						data->size            = fh->Get< uint32_t >( );
						data->opcode          = fh->Get< uint32_t >( ) - 0x04000001;
				...
				script->ActionCount = fh->Get< uint32_t >( );

				if ( script->ActionCount > 0 )
				{
					script->Actions = new CAip::SAipDatum*[ script->ActionCount ];
					for ( uint32_t k = 0; k < script->ActionCount; k++ )
					{
						CAip::SAipDatum* data = new CAip::SAipDatum( );
						data->size            = fh->Get< uint32_t >( );
						data->opcode          = fh->Get< uint32_t >( ) - 0x0B000001;
						data->data            = new uint8_t[ data->size - 8 ];
						fh->Read( data->data, data->size - 8, 1 );
...
void CWorldServer::LoadAipData( )
{
	...
	for ( uint32_t i = 1; i < stbAip->Rows( ); i++ )
	{
		if ( stbAip->Data( i, 0 ) )
		{
