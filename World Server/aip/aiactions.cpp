...
AIACT( 006 )
{
	...
	//	uint32_t entityCount = entity->thisZone->GetEntityCount();
	//	for(uint32_t i = 1; i < MAX_ZONE_CLIENTID; i++){
...
AIACT( 011 )
{
	...
	for ( UINT j = 0; j < map->MonsterList.size( ); j++ )
	{
		...
		if ( chrCount >= data->iNumOfMonster )
			return AI_SUCCESS;
	}
...
AIACT( 017 )
{
	...
	//uint16_t item3;	//Pos: 0x06
	//uint16_t item4;	//Pos: 0x08
	//uint32_t iToOwner;	//Pos: 0x0c
...
AIACT( 028 )
{
	...
	//		return AI_FAILURE;
	//	}
	//	std::map<uint32_t, char*>::iterator triggerITR = server->NpcNames.find(npcId);
	...
	//		pakout.Add<uint32_t>(entity->_ClientID);
...
AIACT( 030 )
{
	...
	return AI_SUCCESS; //TODO: redo this function
}
