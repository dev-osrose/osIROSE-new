...
bool CCharacter::AoeSkill( CSkills* skill, CCharacter* Enemy )
{
	...
	for ( UINT i = 0; i < map->MonsterList.size( ); i++ )
	{
		...
		{
			if ( monster->IsSummon( ) && ( map->allowpvp == 0 || monster->owner == clientid ) )
				continue;
		}
		else
		{
...
void CCharacter::UseAtkSkill( CCharacter* Enemy, CSkills* skill, bool deBuff )
{
    ...
    */
	...
	if ( level_diff >= 10 )
		apchar = (long int)( apchar * 1.2 );
	if ( level_diff >= 20 )
		apchar = (long int)( apchar * 1.4 );
	if ( level_diff >= 30 )
		apchar = (long int)( apchar * 1.7 );
	if ( level_diff >= 40 )
		apchar = (long int)( apchar * 2 );
	if ( level_diff <= 10 )
		apchar = (long int)( apchar * 0.9 );
	if ( level_diff <= 20 )
		apchar = (long int)( apchar * 0.8 );
	...
	if ( skillpower >= 2047 )
		skillpower = 2047;
	if ( this->CharType == 2 )
		skillpower = 30; //aquas kill you in one hit with above calculations
	//Tell enemy he's attacked & add damage & send the dmg packet
	bool bflag = false;
	Enemy->OnBeAttacked( this );
	if ( skillpower >= 5000000 )
		skillpower = 5; //if ever computations got wild- this will avoid having crazy damage -littlelilly
	if ( skill->atkpower <= 5 )
		skillpower = 0; //so that skills having 0 attack have no damage at all - littlelilly
	if ( !Enemy->IsSummon( ) && Enemy->IsMonster( ) )
	{
...
int CCharacter::GetAtkDmg( CCharacter* pDEF, uint16_t wHitCNT, int iSuc )
{
	int curDamage       = 0;
...
int CCharacter::GetMgcDmg( CCharacter* pDEF, uint16_t wHitCNT, int iSuc )
{
	int curDamage       = 0;
	int critSuccessRate = GetCritRate( );
