...
void CWorldServer::pakQuestData( CPlayer* thisclient )
{
	...
	for ( uint32_t i = 0; i < 5; i++ )
		pakout.Add< uint16_t >( thisclient->quest.EpisodeVar[ i ] );
	for ( uint32_t i = 0; i < 3; i++ )
		pakout.Add< uint16_t >( thisclient->quest.JobVar[ i ] );
	for ( uint32_t i = 0; i < 7; i++ )
		pakout.Add< uint16_t >( thisclient->quest.PlanetVar[ i ] );
	for ( uint32_t i = 0; i < 10; i++ )
		pakout.Add< uint16_t >( thisclient->quest.UnionVar[ i ] );

	for ( unsigned i = 0; i < 10; i++ )
	{
		pakout.Add< uint16_t >( thisclient->quest.quests[ i ].QuestID );
		long int Time = 0;
		...
		pakout.Add< uint32_t >( Time ); // Time Left
		for ( uint32_t j = 0; j < 10; j++ )
			pakout.Add< uint16_t >( thisclient->quest.quests[ i ].Variables[ j ] );
		for ( uint32_t j = 0; j < 4; j++ )
			pakout.Add< uint8_t >( thisclient->quest.quests[ i ].Switches[ j ] );
		for ( uint32_t j = 0; j < 6; j++ )
		{
			if ( thisclient->quest.quests[ i ].Items[ j ].itemnum != 0 )
			{
				pakout.Add< uint16_t >( thisclient->quest.quests[ i ].Items[ j ].GetPakHeader( ) );
				pakout.Add< uint32_t >( thisclient->quest.quests[ i ].Items[ j ].GetPakData( ) );
			}
			else
			{
				pakout.Add< uint16_t >( 0 );
				pakout.Add< uint32_t >( 0 );
			}
		}
	}
	for ( unsigned i = 0; i < 0x40; i++ )
		pakout.Add< uint8_t >( thisclient->quest.flags[ i ] );
...
{
	uint8_t  action = pak->GetByte( 0 );
	uint8_t  slot   = pak->GetByte( 1 );
	uint32_t hash   = pak->GetDWord( 2 );
	...
	{
		if ( thisclient->questdebug )
			SendPM( thisclient, "Delete quest - Slot %i", slot );
		for ( uint32_t i = slot; i < 9; i++ )
			thisclient->quest.quests[ i ] = thisclient->quest.quests[ i + 1 ];
		thisclient->quest.quests[ 9 ].Clear( );
		CPacket pakout( 0x730 );
		pakout.Add< uint8_t >( 0x03 );
		pakout.Add< uint8_t >( slot );
		pakout.Add< uint32_t >( hash );
		thisclient->client->SendPacket( &pakout );
		return true;
	}
	if ( action != 3 )
...
bool CWorldServer::pakDoIdentify( CPlayer* thisclient, CPacket* P )
{
	...
	/*	RESETPACKET( pak, 0x702 );
    	...
    	ADDBYTE    ( pak, 0 );
    	thisclient->client->SendPacket( &pak );
    	SendSysMsg( thisclient, "Osirose server made by the community of osrose " );*/
	thisclient->SetStats( );
	return true;
}
...
bool CWorldServer::pakChangeEquip( CPlayer* thisclient, CPacket* P )
{
	...
	if ( !CheckInventorySlot( thisclient, destslot ) )
		return false;
	if ( destslot == 0 )
		destslot = thisclient->GetNewItemSlot( thisclient->items[ srcslot ] );
	if ( destslot == 0xffff )
		return true;
	/*
    ...
    */
	...
	if ( srcslot == 7 || destslot == 7 )
	{
		...
		uint32_t weapontype = EquipList[ WEAPON ].Index[ thisclient->items[ 7 ].itemnum ]->type;
		switch ( weapontype )
		{
		case TWO_HAND_SWORD:
		case SPEAR:
		...
		case DUAL_GUN:
		{
			if ( thisclient->items[ 8 ].itemnum != 0 )
			{
				...
				uint32_t newslot = thisclient->GetNewItemSlot( thisclient->items[ 8 ] );
				if ( newslot == 0xffff )
				{
...
bool CWorldServer::pakStartAttack( CPlayer* thisclient, CPacket* P )
{
	...
	if ( weapontype == 231 && thisclient->items[ 132 ].count < 1 )
		return true;
	else if ( weapontype == 232 && thisclient->items[ 133 ].count < 1 )
		return true;
	else if ( weapontype == 233 && thisclient->items[ 134 ].count < 1 )
		return true;
	else if ( weapontype == 271 && thisclient->items[ 132 ].count < 1 )
		return true;
	...
	{
		if ( map->allowpvp == 0 && character->IsSummon( ) )
			return true;
...
bool CWorldServer::pakNPCBuy( CPlayer* thisclient, CPacket* P )
{
	try
	{
		if ( thisclient->Shop->open )
			return true;
		uint16_t NPC = GETWORD( ( *P ), 0 );
		CNPC* thisnpc = GetNPCByID( NPC, thisclient->Position->Map );
		if ( thisnpc == NULL )
			return true;
		uint8_t buycount = GETBYTE( ( *P ), 2 );
		uint8_t sellcount = GETBYTE( ( *P ), 3 );
		uint8_t ncount = 0;
		...
		{
			uint8_t tabid = GETBYTE( ( *P ), 8 + ( i * 4 ) );
			uint8_t itemid = GETBYTE( ( *P ), 9 + ( i * 4 ) );
			uint16_t count = GETWORD( ( *P ), 10 + ( i * 4 ) );
			unsigned sellid = 0;
			switch ( tabid )
			{
			case 0:
			...
			while ( tmpcount > 0 )
			{
				...
				case 12:
				{
					...
					uint8_t  values[ 11 ] = {0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32};
					uint32_t type         = 0;
					uint32_t bprice       = 0;
					uint32_t pricerate    = 0;
					uint32_t pvalue       = 0;
					if ( thisitem.itemtype == 10 )
					{
		...
		{
			uint8_t slotid = GETBYTE( ( *P ), 8 + ( buycount * 4 ) + ( i * 3 ) );
			uint16_t count = GETWORD( ( *P ), 9 + ( buycount * 4 ) + ( i * 3 ) );
...
bool CWorldServer::pakTradeAction( CPlayer* thisclient, CPacket* P )
{
	...
	case 1:
		...
		for ( int i = 0; i < 11; i++ )
			thisclient->Trade->trade_count[ i ] = 0;
		for ( int i = 0; i < 10; i++ )
			thisclient->Trade->trade_itemid[ i ] = 0;
		for ( int i = 0; i < 11; i++ )
			otherclient->Trade->trade_count[ i ] = 0;
		for ( int i = 0; i < 10; i++ )
			otherclient->Trade->trade_itemid[ i ] = 0;
		break;
	case 3:
		RESETPACKET( pak, 0x7c0 );
		ADDBYTE( pak, 3 );
...
bool CWorldServer::pakTradeAdd( CPlayer* thisclient, CPacket* P )
{
	if ( thisclient->Shop->open )
		return true;
	uint8_t islot = GETBYTE( ( *P ), 0 );
	uint16_t slotid = GETWORD( ( *P ), 1 );
	uint32_t count = GETDWORD( ( *P ), 3 );
	CPlayer* otherclient = GetClientByID( thisclient->Trade->trade_target, thisclient->Position->Map );
	if ( otherclient == NULL )
		return true;
...
bool CWorldServer::pakLevelUpSkill( CPlayer* thisclient, CPacket* P )
{
	...
	if ( thisskill == NULL || nextskill == NULL )
		return true;
	...
	for ( uint8_t i = 0; i < 2; i++ )
	{
		...
		case sLevel:
			...
			break;
		case sStrength:
			if ( nextskill->reqam[ i ] > thisclient->Attr->Str )
				fail = true;
			break;
		case sDexterity:
			if ( nextskill->reqam[ i ] > thisclient->Attr->Dex )
				fail = true;
			break;
		case sIntelligence:
			if ( nextskill->reqam[ i ] > thisclient->Attr->Int )
				fail = true;
			break;
		case sConcentration:
			if ( nextskill->reqam[ i ] > thisclient->Attr->Con )
				fail = true;
			break;
		case sCharm:
			if ( nextskill->reqam[ i ] > thisclient->Attr->Cha )
				fail = true;
			break;
		case sSensibility:
			if ( nextskill->reqam[ i ] > thisclient->Attr->Sen )
				fail = true;
			break;
		}
	}
	...
	for ( uint8_t i = 0; i < 4; i++ )
	{
		if ( nextskill->c_class[ i ] > 0 )
		{
			...
			{
			case cSoldierJob:
				if ( ( thisclient->CharInfo->Job ) / 100 == 1 )
					passClass = true;
				break;
			case cMuseJob:
				if ( ( thisclient->CharInfo->Job ) / 100 == 2 )
					passClass = true;
				break;
			case cHawkerJob:
				if ( ( thisclient->CharInfo->Job ) / 100 == 3 )
					passClass = true;
				break;
			case cDealerJob:
				if ( ( thisclient->CharInfo->Job ) / 100 == 4 )
					passClass = true;
				break;
			case cKnightJob:
				if ( thisclient->CharInfo->Job == 121 )
					passClass = true;
				break;
			case cChampionJob:
				if ( thisclient->CharInfo->Job == 122 )
					passClass = true;
				break;
			case cMageJob:
				if ( thisclient->CharInfo->Job == 221 )
					passClass = true;
				break;
			case cClericJob:
				if ( thisclient->CharInfo->Job == 222 )
					passClass = true;
				break;
			case cRaiderJob:
				if ( thisclient->CharInfo->Job == 321 )
					passClass = true;
				break;
			case cScoutJob:
				if ( thisclient->CharInfo->Job == 322 )
					passClass = true;
				break;
			case cBourgeoisJob:
				if ( thisclient->CharInfo->Job == 421 )
					passClass = true;
				break;
			case cArtisanJob:
				if ( thisclient->CharInfo->Job == 422 )
					passClass = true;
				break;
			}
			if ( passClass )
	...
	if ( pos > 59 )
		thisclient->pskilllvl[ pos - 60 ] += 1;
	else if ( pos > 29 )
		thisclient->askilllvl[ pos - 30 ] += 1;
...
{
	uint32_t result = GETDWORD( ( *P ), 0x00 );
	BEGINPACKET( pak, 0x07d8 );
	ADDWORD( pak, thisclient->clientid );
	ADDDWORD( pak, result );
	SendToVisible( &pak, thisclient );
...
bool CWorldServer::pakStorage( CPlayer* thisclient, CPacket* P )
{
	...
	case 0x00:
	{
		uint32_t nstorageitems = thisclient->Getnstorageitems( thisclient );
...
bool CWorldServer::pakChangeStorage( CPlayer* thisclient, CPacket* P )
{
    ...
    */
	...
	uint32_t bpricerate = 0; // Item Price Rate (from STB)
	uint32_t bfee       = 0; // Storage Fee     (Calculated)
	uint8_t action = GETBYTE( ( *P ), 0 );
	...
	case 0x01: //Withdraw
	{
		uint8_t storageslot = GETBYTE( ( *P ), 1 );
		CItem clientitem = GetItemByHeadAndData( GETWORD( ( *P ), 2 ), GETDWORD( ( *P ), 4 ) );
...
bool CWorldServer::pakOpenShop( CPlayer* thisclient, CPacket* P )
{
	...
	if ( thisclient->Shop->open )
		return true;
	uint8_t nselling = GETBYTE( ( *P ), 0 );
	uint8_t nbuying = GETBYTE( ( *P ), 1 );
	...
	for ( int i = 0; i < nbuying; i++ )
	{
		...
		uint32_t head = GETWORD( ( *P ), n + 1 );
		uint32_t data = GETDWORD( ( *P ), n + 3 );
...
{
	uint16_t otherclientid = GETWORD( ( *P ), 0 );
		     ...
		     otherclient->items[ invslot ].itemtype < 14 )
		{
			if ( otherclient->items[ invslot ].count <= count )
			{
				ClearItem( otherclient->items[ invslot ] )
				    otherclient->Shop->SellingList[ slot ]
				        .slot                                = 0;
...
bool CWorldServer::pakModifiedItem( CPlayer* thisclient, CPacket* P )
{
	...
	case 0x01: //Gemming
	{
		uint8_t destslot = GETBYTE( ( *P ), 1 );
		uint8_t srcslot = GETBYTE( ( *P ), 2 );
		if ( !CheckInventorySlot( thisclient, destslot ) )
			return false;
		if ( !CheckInventorySlot( thisclient, srcslot ) )
			return false;
	...
	{
		uint8_t item = GETBYTE( ( *P ), 3 );
		uint8_t material1 = GETBYTE( ( *P ), 4 );
		uint8_t material2 = GETBYTE( ( *P ), 5 );
		uint8_t material3 = GETBYTE( ( *P ), 6 );
		...
		if ( nextlevel > 9 )
			return true; //if a client were able to try and upgrade an lvl 9 grade item this would probably lockup this client
...
{
	Log( MSG_INFO, "Repair hammer. %02x %02x %02x %02x", GETBYTE( ( *P ), 0 ), GETBYTE( ( *P ), 1 ), GETBYTE( ( *P ), 2 ), GETBYTE( ( *P ), 3 ) );
	uint32_t repairhammer = GETBYTE( ( *P ), 0 );
	uint32_t repairitem = GETBYTE( ( *P ), 2 );
...
// Cost are calculated in function Repairprice, located in Extrafunctions.cpp
{
	...
	case 0x43: //NPC Repair Item / Town
	{
		...
		uint32_t bprice   = EquipList[ thisitem.itemtype ].Index[ thisitem.itemnum ]->price;
		// Get the repair cost.

		uint32_t bcost = Repairprice( bprice, thisitem.durability, thisitem.lifespan );
...
bool CWorldServer::pakRideRequest( CPlayer* thisclient, CPacket* P )
{
	...
	{
		uint16_t tclientid = GETWORD( ( *P ), 1 );
		uint16_t oclientid = GETWORD( ( *P ), 3 );
	...
	case 0x02: //Accept
	{
		uint16_t oclientid = GETWORD( ( *P ), 1 );
		uint16_t tclientid = GETWORD( ( *P ), 3 );
		if ( tclientid != thisclient->clientid )
			return true;
		CPlayer* otherclient = GetClientByID( oclientid, thisclient->Position->Map );
		if ( otherclient == NULL )
...
{
	uint8_t weight = GETBYTE( ( *P ), 0 ) & 0xff;
	if ( weight > 110 )
		thisclient->Status->CanAttack = false;
	else
		thisclient->Status->CanAttack = true;
...
{
	if ( !thisclient->Session->isLoggedIn )
		return true;
	uint8_t action = GETBYTE( ( *P ), 0 );
	switch ( action )
	{
