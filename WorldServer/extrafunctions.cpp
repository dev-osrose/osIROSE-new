...
bool CWorldServer::IsVisible( CPlayer* thisclient, CMonster* thismon )
{
	...
	{
		if ( thismon->clientid == thisclient->VisibleMonsters.at( j ) )
			return true;
	}
	return false;
}
...
bool CWorldServer::IsVisible( CPlayer* thisclient, CDrop* thisdrop )
{
	...
	{
		if ( thisdrop == thisclient->VisibleDrops.at( j ) )
			return true;
	}
	return false;
}
...
bool CWorldServer::IsVisible( CPlayer* thisclient, CNPC* thisnpc )
{
	...
	{
		if ( thisnpc == thisclient->VisibleNPCs.at( j ) )
			return true;
	}
	return false;
}
...
fPoint CWorldServer::RandInTriangle( fPoint p1, fPoint p2, fPoint p3 )
{
	...
	while ( a + b > 1 )
	{
		a = (float)( rand( ) * 1.0 / RAND_MAX );
		b = (float)( rand( ) * 1.0 / RAND_MAX );
	}
	float c     = 1 - a - b;
...
fPoint CWorldServer::RandInPoly( fPoint p[], int pcount )
{
	...
	for ( tnum = 1; tnum < pcount - 1; tnum++ )
	{
		if ( tval <= areas[ tnum ] && tval > areas[ tnum - 1 ] )
			break;
	}
...
{
	if ( thisclient->Position->Map != thismon->Position->Map )
		return false;
...
{
	if ( thisclient->Position->Map != otherclient->Position->Map )
		return false;
...
CUseInfo* CWorldServer::GetUseItemInfo( CPlayer* thisclient, unsigned int slot )
{
	...
	case 313: //Magic Item [scrolls/summons/fireworks/etc]
	{
		    ...
		    if ( ( useitem->itemnum > 400 && useitem->itemnum < 440 ) || useitem->itemnum == 942 )
		{
			...
			useitem->usescript = 5;
			useitem->usetype   = UseList.Index[ useitem->itemnum ]->useeffect[ 1 ];
			if ( useitem->itemnum == 439 )
			{
				useitem->usevalue = 965;
			}
			else if ( useitem->itemnum == 942 )
			{
				useitem->usevalue = 1472;
			}
			else
			{
				useitem->usevalue = useitem->itemnum + 500;
			}
		}
		else // Snowball
...
uint32_t CWorldServer::RandNumber( uint32_t init, uint32_t range, uint32_t seed )
{
	if ( seed != 0 )
		srand( seed );
	if ( range > RAND_MAX )
	{
		uint32_t max = (uint32_t)GServer->round( sqrt( (double)range ) );
		uint32_t num1 = init + int( max * rand( ) / ( RAND_MAX + 1.0 ) );
		uint32_t num2 = init + int( max * rand( ) / ( RAND_MAX + 1.0 ) );
		uint32_t num3 = init + int( max * rand( ) / ( RAND_MAX + 1.0 ) );
		uint32_t res = ( num1 * num2 ) + num3;
		return res > range ? range : res;
	}
	else
	{
		uint32_t ranum = init + int( range * rand( ) / ( RAND_MAX + 1.0 ) );
		return ranum;
	}
}

// Check if is a valid item
bool CWorldServer::IsValidItem( uint32_t type, uint32_t id )
{
	if ( type == 0 || id == 0 )
		return false;
...
uint32_t CWorldServer::GetLevelGhost( uint32_t map, uint32_t level )
{
	uint32_t montype = 0;
	montype = (uint32_t)floor( (double)( level - 19 ) / 4 ) + 711;
	if ( map >= 1 && map <= 50 )
	{
		if ( level < 20 )
			return 711;
