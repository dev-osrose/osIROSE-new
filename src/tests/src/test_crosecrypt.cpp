...
TEST( TestRoseCrypt, TestEncryptData )
{
	...
	EXPECT_EQ( sizeof(pakChannelList_Req), g_Crypt.decodeServerHeader( (unsigned char*)&pak.Buffer ) );
...
TEST( TestRoseCrypt, TestDecryptData )
{
	...
	//EXPECT_EQ( sizeof(pakChannelList_Req), g_Crypt.decodeClientHeader( (unsigned char*)&pak.Buffer ) );
