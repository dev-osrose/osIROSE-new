...
void Combat::check_for_level_up(EntitySystem& entitySystem, Entity entity, uint16_t sourceId) {
  ...
  while ((int64_t)attackerLevel.xp >= exp_required) {
    ...
    basicInfo.statPoints += 9 + attackerLevel.level - ((attackerLevel.level - 1) / 5); //changed by davidixx
    basicInfo.skillPoints += (attackerLevel.level / 2) + 1; //changed by davidixx

    exp_required = get_exp_to_level(attackerLevel.level + 1);
  }

  if (leveled) {
    updateStats(entitySystem, entity);
    auto pLevel =
        SrvLevelup::create(basicInfo.id, attackerLevel.level, attackerLevel.xp, /*changed by davidixx*/basicInfo.statPoints, /*changed by davidixx*/basicInfo.skillPoints);
...
std::pair<float, float> Combat::get_range_position(const EntitySystem& entitySystem, Entity character, Entity target,
                                                   float range) {
  auto logger = Core::CLog::GetLogger(Core::log_type::GENERAL).lock();
  ...
  logger->debug("out of range, returning x,y {},{}", (vector.first * range) + target_pos.x,
                (vector.second * range) + target_pos.y);
...
void Combat::updateStats(EntitySystem& entitySystem, Entity entity) {
  Calculations::get_maxhp(entitySystem, entity);
  Calculations::get_maxmp(entitySystem, entity);
  Calculations::get_maxweight(entitySystem, entity);
}
...
void Combat::update(EntitySystem& entitySystem, Entity entity, uint32_t dt) {
  ...
  if (values.regenDt >= 4000)  // Regen happens every 4 seconds
  {
    ...
    if (life.hp > 0 && life.hp != life.maxHp) {
      ...
      if (life.hp > life.maxHp) life.hp = life.maxHp;
    ...
    if (entitySystem.has_component<Component::Magic>(entity)) {
      ...
      if (magic.mp != magic.maxMp) {
        ...
        if (magic.mp > magic.maxMp) magic.mp = magic.maxMp;
...
//        entitySystem.send_nearby(entity, p);
    ...
    if (life.hp <= 0) {
      if (entitySystem.has_component<Component::Mob>(entity)) {
        ...
        for (auto& attack_log : queuedDamage.damage_log_) {
          ...
          if (attack_log.value_ > (life.maxHp * 1.15f)) attack_log.value_ = (life.maxHp * 1.15f);
  ...
  if (values.combatDt > 0) values.combatDt -= dt;
  ...
  if (entitySystem.has_component<Component::Target>(entity)) {
    ...
    if (((!entitySystem.has_component<Component::Npc>(target.target) &&
         ...
         (false && targetBasicInfo.teamId != basicInfo.teamId)))
    // TODO:: Check if this map has PVP turned on and the target player isn't on my team
    {
...
RoseCommon::Entity Combat::get_saved_spawn(EntitySystem& entitySystem, RoseCommon::Entity player) {
  ...
  for (RoseCommon::Entity entity :
    ...
    if (spawninfo.type == Component::PlayerSpawn::RESPAWN_POINT) {
      ...
      if (spawnPosition.map == position.spawn) return entity;
    }
  }
...
void Combat::drop_loot(EntitySystem& entitySystem, RoseCommon::Entity entity, RoseCommon::Entity owner) {
  ...
  if (Core::Random::getInstance().get_uniform(0, 100) <= lua_data->get_drop_money()) {
    ...
    Items::drop_item(entitySystem, entitySystem.create_zuly(Core::Random::getInstance().get_normal(amount, deviation)),
                     posX, posY, owner);
  }
}
