...
// Raven0123
void
CServerSocket::CryptISCPak ( char* pak )
{
	return;
...
CServerSocket::ServerLoop ()
{
	...
	do
	{
...
#ifdef _WIN32
				NewSocket = accept ( sckISC, (sockaddr*) &ClientInfo,
														 (int*) &clientinfolen );
#else
		...
		if ( FD_ISSET( sock, &fds ) )
		{
			int clientinfolen = sizeof(sockaddr_in);
#ifdef _WIN32
			NewSocket = accept ( sock, (sockaddr*) &ClientInfo,
													 (int*) &clientinfolen );
#else
...
void
CServerSocket::AddUser ( SOCKET sock, sockaddr_in* ClientInfo, bool server )
{
	ConnectedClients++;
	...
	if ( server == true )
	{
		...
		Log ( MSG_INFO, "Server connected from %s",
					inet_ntoa ( ClientInfo->sin_addr ) );
	}
	else
	{
		thisclient->isserver = false;
		Log ( MSG_INFO, "User connected from %s",
					inet_ntoa ( ClientInfo->sin_addr ) );
	}
}
...
// Disconnect our user
void
CServerSocket::DisconnectClient ( CClientSocket* thisclient )
{
	ConnectedClients--;
...
void
CServerSocket::DeleteClientSocket ( CClientSocket* thisclient )
{
...
void
CServerSocket::OnServerStep ()
{
}

// This function is called just before the server starts
bool
CServerSocket::OnServerReady ()
{
	return true;
}
