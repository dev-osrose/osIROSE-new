...
bool CCharServer::SendClanInfo( CCharClient* thisclient )
{
	if ( thisclient->clanid > 0 )
	{
		...
		if ( thisclan != NULL )
		{
			...
			ADDBYTE( pak, thisclient->clan_rank ); // Clan rank (0 = red rokie /
			                                       // 6 = master)
			...
			for ( uint32_t i = 0; i < thisclan->ClanMembers.size( ); i++ )
			{
				...
				if ( otherclient != NULL )
					ChangeClanStatus(
					    thisclient, otherclient,
					    thisclient->channel ); // send channel here
			}
		}
	}
	return true;
...
bool CCharServer::ChangeClanStatus( CCharClient* thisclient,
                                    CCharClient* otherclient, int channel )
{
...
bool CCharServer::pakClanChat( CCharClient* thisclient, CPacket* P )
{
	...
	char* text = new ( nothrow ) char[ P->Header.Size ];
	if ( text == NULL )
	{
	...
	memcpy( text, &P->Buffer, P->Header.Size );
...
bool CCharServer::pakClanManager( CCharClient* thisclient, CPacket* P )
{
	...
	{
		char* nick = new ( nothrow ) char[ P->Header.Size ];
		if ( nick == NULL )
		{
			Log( MSG_ERROR, "Error allocing memory" );
			return false;
		...
		memcpy( nick, &P->Buffer[ 1 ], P->Header.Size );
	...
	case 0x03: // Kick from clan
	{
		...
		for (uint32_t i = 0; i < thisclan->ClanMembers.size( ); i++)
		{
			...
			{
				thisclan->ClanMembers.erase( thisclan->ClanMembers.begin( ) +
				                             i );
				break;
			}
		}
		...
		if ( otherclient != nullptr )
		{
			...
			{
				Log( MSG_ERROR, "Check serverid in worldserver.conf it should "
				                "be the same as ID in channels in the mysql "
				                "database %i",
				     thisclient->channel );
			}
		...
		if ( !DB->QExecute( "UPDATE characters SET clanid=0 AND clan_rank=1 "
		                    "WHERE char_name='%s'",
		                    nick ) )
		{
			delete[] nick;
	...
	{
		bool  online = true;
		char* nick   = new ( nothrow ) char[ P->Header.Size - 7 ];
		if ( nick == NULL )
		{
			Log( MSG_ERROR, "Error allocing memory" );
			return false;
		...
		{
			online                                   = false;
			std::unique_ptr< sql::ResultSet > result = DB->QStore( "SELECT clan_rank,classid,level FROM "
			...
			clan_rank = result->getInt( "clan_rank" );
			job       = result->getInt( "classid" );
	...
	case 0x06: // Change news
	{
		if ( thisclient->clan_rank < 5 )
			return true;
		char* news = new ( nothrow ) char[ P->Header.Size - 7 ];
		if ( news == NULL )
		{
			Log( MSG_ERROR, "Error allocing memory" );
			return false;
		}
		memcpy( news, &P->Buffer[ 1 ], P->Header.Size - 7 );
		CClans* thisclan = GetClanByID( thisclient->clanid );
		if ( thisclan != NULL )
		{
			...
			ADDBYTE( pak, 0x00 );
			SendToClanMembers( thisclan->id, &pak );
			if ( !DB->QExecute( "UPDATE list_clan SET news='%s' WHERE id=%i",
			                    thisclan->news, thisclan->id ) )
				return false;
		}
		delete[] news;
	}
	...
	case 0x07: // Leave Clan
	{
		...
		if ( !DB->QExecute( "UPDATE characters SET clanid=0 , clan_rank=0 "
		                    "WHERE char_name='%s'",
		                    thisclient->charname ) )
			return false;
		RESETPACKET( pak, 0x7e1 ); // Update world clan information
	...
	case 0x09: // Give Master
	{
		...
		{ //   0     1
			std::unique_ptr< sql::ResultSet > result = DB->QStore(
			    "SELECT classid,level FROM characters where char_name='%s'",
			    nick );
	...
	case 0x0a: // disorganize clan
	{
		...
		if ( thisclan == NULL )
			return true;
		if ( thisclan->ClanMembers.size( ) >
		     1 ) // Can't be deleted if there are members
			return true;
		if ( !DB->QExecute( "UPDATE characters SET clanid=0 AND clan_rank=1 "
		                    "WHERE clanid=%i",
		                    thisclient->clanid ) )
	...
	case 0x0c: // invitation accepted
	{
		...
		if ( otherclient != NULL )
		{
			...
			thisclient->clan_rank = 1;
			if ( !DB->QExecute( "UPDATE characters SET clanid=%i , clan_rank=1 "
			                    "WHERE char_name='%s'",
			                    thisclient->clanid, thisclient->charname ) )
			{
				delete[] nick;
				return false;
			}
			...
			if ( thisclan == NULL )
			{
				Log( MSG_ERROR, "Clan with id %d doesn't exist",
				     thisclient->clanid );
				delete[] nick;
				return true;
			}
	...
	break;
	case 0xfa: // message from world server to load the new clan information
	{
		...
		//     0    1  2    3    4   5   6       7
		std::unique_ptr< sql::ResultSet > result = DB->QStore( "SELECT id,logo,back,name,cp,grade,slogan,news "
		                                                       "FROM list_clan where id=%i",
		                                                       id );
		if ( result == NULL )
			return false;
		...
		newclan->id    = result->getInt( "id" );
		newclan->logo  = result->getInt( "logo" );
		newclan->back  = result->getInt( "back" );
		newclan->cp    = result->getInt( "cp" );
		newclan->grade = result->getInt( "grade" );
		strncpy( newclan->name, result->getString( "name" ).c_str( ), 16 );
		strncpy( newclan->slogan, result->getString( "slogan" ).c_str( ), 29 );
		strcpy( newclan->news, result->getString( "news" ).c_str( ) );
	...
	default:
		Log( MSG_INFO, "Clan action unknown %i ", action );
		break;
	}
	return true;
}
	...
	if ( thisclan != NULL )
	{
		...
		for (uint32_t i = 0; i < thisclan->ClanMembers.size( ); i++)
		{
			...
			if ( otherclient != NULL )
			{
				...
				ADDBYTE( pak,
				         otherclient->channel ); // channel (0x01 = channel 1)
				ADDWORD( pak, 0x0000 );
				ADDWORD( pak, 0x0000 );
...
bool CCharServer::ClanLogout( CCharClient* thisclient )
{
	...
	if ( thisclan != NULL )
	{
		for (uint32_t i = 0; i < thisclan->ClanMembers.size( ); i++)
		{
			...
			if ( otherclient != NULL )
			{
				ChangeClanStatus( thisclient, otherclient,
				                  0xff ); // send channel here
			}
		}
	}
	return true;
...
bool CCharServer::pakUploadCM( CCharClient* thisclient, CPacket* P )
{
	...
	DB->QExecute( "UPDATE list_clan SET logo=%i,back=0 WHERE id=%i",
	              thisclan->logo, thisclient->clanid );
...
bool CCharServer::pakDownloadCM( CCharClient* thisclient, CPacket* P )
{
	...
	std::unique_ptr< sql::ResultSet > result = DB->QStore( "SELECT logo FROM list_clan WHERE id=%i", clanid );
	...
	unsigned int cmid = result->getInt( "logo" );
	char         filename[ 30 ];
