...
bool
CCharServer::pakDoIdentify(CCharClient* thisclient, CPacket* P)
{
	...
	{
		Log(MSG_HACK,
			"Someone tried to connect to char server with an invalid account");
		return false;
	}
	else
	{
...
bool
CCharServer::pakGetCharacters(CCharClient* thisclient, CPacket* P)
{
	if (!thisclient->isLoggedIn)
		return false;
...
bool
CCharServer::pakRequestWorld(CCharClient* thisclient, CPacket* P)
{
	if (!thisclient->isLoggedIn)
		return false;
	MYSQL_ROW row;
	...
	if (!DB->QExecute("UPDATE accounts SET lastchar='%s' WHERE id=%i",
		...
		return false;
	result = DB->QStore("SELECT host,port FROM channels WHERE id=%i",
		thisclient->channel);
	if (result == NULL)
		return false;
	...
	result =
		DB->QStore(
		"SELECT clanid,clan_rank,id,level,classid FROM characters WHERE char_name='%s'",
		thisclient->charname);
	if (result == NULL)
		return false;
	if (mysql_num_rows(result) != 1)
	...
	//              //          0        1
	result = DB->QStore(
		"SELECT idfriend,namefriend FROM list_friend WHERE id=%i",
		thisclient->charid);
	if (result == NULL)
		return false;
...
// Create a character
bool
CCharServer::pakCreateChar(CCharClient* thisclient, CPacket* P)
{
	if (!thisclient->isLoggedIn)
		return false;
	...
	{
		if (!DB->QExecute(
			"INSERT INTO items VALUES(%i,222,2,0,40,100,12,1,0,0,0,0)", mid))
			return false;
	}
	else
	{
...
// Connect WorldServer
bool
CCharServer::pakWSReady(CCharClient* thisclient, CPacket* P)
{
	...
	{
		Log(MSG_HACK,
			"WorldServer Tried to connect CharServer with wrong password ");
		return true;
	}
	...
	if (connect(newchannel->sock, (SOCKADDR*)&ain,
...
// Login Connected
bool
CCharServer::pakLoginConnected(CCharClient* thisclient, CPacket* P)
{
	if (thisclient->isLoggedIn)
		return false;
...
bool
CCharServer::pakWSCharSelect(CCharClient* thisclient, CPacket* P)
{
...
// ???
bool
CCharServer::pak7e5(CCharClient* thisclient, CPacket* P)
{
	BEGINPACKET(pak, 0);
	if (!thisclient->isLoggedIn)
		return false;
...
// Create a chatroom?
bool
CCharServer::pakChatrooms(CCharClient* thisclient, CPacket* P)
{
	if (!thisclient->isLoggedIn)
		return false;
	return true;
...
bool
CCharServer::pakDeleteChar(CCharClient* thisclient, CPacket* P)
{
	if (!thisclient->isLoggedIn)
		return false;
	...
	case 0x00:	//Resurect
	{
					DeleteTime = 0;
					if (!DB->QExecute(
						" UPDATE characters SET deletetime=0 WHERE char_name='%s'",
						(char*)&P->Buffer[2]))
						return false;
	}
		break;
	case 0x01:	//Delete
	{
					...
					if (!DB->QExecute(
						" UPDATE characters SET deletetime=%i WHERE char_name='%s'",
...
bool
CCharServer::pakUpdateLevel(CCharClient* thisclient, CPacket* P)
{
	if (!thisclient->isLoggedIn)
		return false;
...
bool
CCharServer::pakLoginDSClient(CCharClient* thisclient, CPacket* P)
{
	...
	case 1:
	{
			  ...
			  else
			  {
				  Log(
					  MSG_ERROR,
					  "Check serverid in worldserver.conf it should be the same as ID in channels in the mysql database %i",
					  thisclient->channel);
			  }
	}
		break;
